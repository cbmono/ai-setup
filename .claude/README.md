...
Two verified behaviors that shape guidance elsewhere: background subagents **do** inherit the browser connection (it isn't foreground-only), but each gets its **own tab group** rather than the human's open tabs — so agents must navigate from an explicit URL. `settings.json` leaves these tools **prompting** by default. Note that this is a *harness-level* gate underneath ai-bridge's `autonomy`: a project on `yolo` permits browser writes at the policy level, but a background agent will still stall on a permission prompt unless `mcp__claude-in-chrome__*` is allowlisted in the consumer's own `settings.local.json`. Say so wherever autonomous browser work is documented — the two layers are independent and it's a confusing failure otherwise.
