...
4. **Resolve capabilities & kind-specific fields (capabilities first).**
   ...
   **a. Capabilities (flags-first, else ask).** Settle `kind`, `autonomy`, `clis`, and
   ...
   external PR reviewer** (e.g. CodeRabbit) in the repo's branch protection: the PM only
   auto-merges when a required independent review + green CI are present, and otherwise
   falls back to `yolo` (surfaces the PR for you). Flag this at scaffold time so the
   human can set branch protection.
