...
3. **Dispatch `ready → in-progress`.** **Build tasks only.** Skip any `kind: research`
   ...
   **Model routing.** Route each dispatch to a cost-appropriate model. Read the
   ...
   `project-manager` → `apex`/`fable` by default, set by `/pm-loop` when it spawned you;
   you route workers *down* from there.) For each dispatch: start from the assignee's default tier
