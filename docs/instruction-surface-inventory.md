...
| 4 | **One active `/pm-loop` per clone**, plus the shared-bundle case (`owner` gates dispatch; identity in `instance.config.local.json`) | ALWAYS | No cross-session lock exists; a second loop **on one clone** double-dispatches and races pushes. Two humans with two clones is the supported case and needs the `owner`/identity rules in context before the first dispatch | inline |
